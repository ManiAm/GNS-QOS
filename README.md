...
- **[Quality of Service (QoS)](docs/01_QOS.md):** Foundational concepts — the four network metrics (packet loss, latency, jitter, throughput), why Best-Effort delivery fails for sensitive workloads, and how QoS solves the problem.
- **[QoS Service Models and Switch Implementation](docs/02_SERVICE_MODELS.md):** Industry standards and switch internals — IntServ vs. DiffServ, DSCP and Per-Hop Behaviors, the full ingress-to-egress DiffServ pipeline (classification, metering, marking, policing, and shaping).
- **[Active Queue Management](docs/02a_AQM.md):** Egress congestion actions — RED, ECN, WRED (the unified WRED/ECN profile), Packet Trimming (trim eligibility, DSCP rewrite, priority promotion, end-to-end workflow), and ASIC cell internals (cell size, embedded vs. separate metadata, SOP/MOP/EOP).
